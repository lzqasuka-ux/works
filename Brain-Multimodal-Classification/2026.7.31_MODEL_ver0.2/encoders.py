...
from proto_head import DiseasePrototypeHead
...
class GATEncoder(nn.Module):
    ...
    def __init__(self, n_nodes=246, n_timepoints=200, hidden_dim=64,
        ...
        if use_fc_input:
            ...
            self.node_proj = nn.Linear(n_timepoints, hidden_dim)
    ...
    def forward(self, x):
        ...
        if self.use_fc_input:
            ...
            x = self.node_proj(x)           # (B, N, T) → (B, N, hidden)
        x = x + self.roi_embed              # (B, N, hidden)  强制区分节点身份
...
class LearnableFC(nn.Module):
    ...
    def forward(self, x):
        ...
        # 投影: (B, N, T) → (B, N, d)，不做 LayerNorm 保留脑区间差异
        q = self.Q(x)  # (B, N, d)
        k = self.K(x)  # (B, N, d)

        # 点积 → 逐样本 max-abs 归一化到 [-1, 1] → 对称化
        d = q.size(-1)
        fc = torch.matmul(q, k.transpose(1, 2)) / (d ** 0.5)      # (B, N, N)
        scale = fc.abs().max(dim=-1, keepdim=True)[0] \
                         .max(dim=-2, keepdim=True)[0].clamp_min(1e-6)
        fc = fc / scale                                             # [-1, 1]
