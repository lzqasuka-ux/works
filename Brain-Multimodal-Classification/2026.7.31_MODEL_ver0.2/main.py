...
import os, json, argparse, glob, time, sys
from datetime import datetime
from torch.utils.data import DataLoader, Subset, ConcatDataset
import torch.optim as optim

from encoders import BrainDiseaseModel, SMRIEncoder3D, MorphEncoder, GATEncoder
from losses import SupConLoss, DomainAwareSupConLoss, orthogonality_loss
from domain_modules import (DomainDiscriminator, HCDomainDiscriminator,
                            DomainWrapper, CombatMorphWrapper, CombatFcWrapper)
from sampler import BalancedBatchSampler
from train_utils import (train_one_epoch, validate, compute_group_distances, compute_metrics)
...
def main(args):
    ...
    # Determine global max T — fast header-only scan, no heavy data loading
    ...
    model = BrainDiseaseModel(num_classes=args.num_classes, latent_dim=args.latent_dim, dropout=args.dropout,
                              n_timepoints_bn=global_max_T_bn, use_fc_input=use_fc_input).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        compute_group_distances(model, val_loader, device)
        ...
        if val_loss < best_loss:
            ...
            for old in glob.glob(args.save_path.replace('.pth', '_epoch*.pth')):
                os.remove(old)
            save_name = os.path.join(out_dir, os.path.basename(args.save_path)).replace('.pth',
            ...
            if hasattr(model, 'classifier_proto'):
                ...
                torch.save(proto_dict, "best_prototypes.pt")
    ...
    if not best_files:
        print("警告: 未找到最佳模型文件")
        return
    best_file = sorted(best_files, key=lambda x: float(x.split('_acc')[-1].replace('.pth','')), reverse=True)[0]
