...
from losses import (orthogonality_loss, mmd_loss, consistency_loss,
                    prototype_loss, token_diversity_loss)
from domain_modules import GradientReversalLayer, HCDomainDiscriminator
...
def train_one_epoch(model, loader, criterion, optimizer, device, epoch,
    ...
    for i, batch in enumerate(loader):
        ...
        # Per-ROI z-score: 消除 COBRE(原始BOLD) vs ADHD(百分信号) 的量纲差异
        roi_bn = (roi_bn - roi_bn.mean(dim=-1, keepdim=True)) / (roi_bn.std(dim=-1, keepdim=True) + 1e-8)
...
def compute_group_distances(model, loader, device):
    ...
    try:
        ...
        sil = silhouette_score(X, y, random_state=42)
